...
export type InterestGrabber<TYPE> = (current: EstimationState, previous?: EstimationState) => TYPE | void
export type InterestComparable<TYPE> = (current: TYPE | undefined, next: TYPE | undefined) => boolean
export type Changer = (current: EstimationState) => EstimationState;

const listeners = new Set<(current: EstimationState, previous?: EstimationState) => void>()
...
export const stateInitialization = new Promise((resolve) => {
    ...
    const tryAgain = () => {
        ...
        socket.send(`{"$patch":${JSON.stringify(compare({}, initialEstimationState))}}`)
        setTimeout(tryAgain, 50)
    }
    setTimeout(tryAgain, 0)
    })
...
export function useEstimationEvent<RESULT>(getRerendableValue: InterestGrabber<RESULT | undefined>, options?: EstimationEventOptions<RESULT>
